...
const checks = [
  ...
  [html.includes('theme.css?v=4') && html.includes('styles.css?v=42') && privacy.includes('styles.css?v=39'), "continuous background theme assets are cache-busted"],
  ...
  [packageJson.dependencies?.three === "0.185.1" && threeRuntimePresent && html.includes('id="universe-render"') && html.includes('type="module" src="three-universe.js?v=6"') && universe.includes('from "./vendor/three.module.min.js"'), "pinned, locally hosted Three.js runtime powers the 3D universe"],
  ...
  [(universe.match(/radius: \d+\.\d+/g) || []).length === 4 && universe.includes("radius: 1.35") && universe.includes("radius: 2.62") && universe.includes("new THREE.LineLoop") && universe.includes("new THREE.Raycaster") && universe.includes("ResizeObserver") && css.includes(".has-three-universe .solar-orbits { visibility: visible; opacity: 1; }") && css.includes(".has-three-universe .gravity-core .earth-globe { visibility: hidden; opacity: 0; }") && script.includes('CustomEvent("motion:change"') && universe.includes('addEventListener("motion:change"'), "3D worlds preserve the original luminous orbit layers, Earth ring, raycast interaction, and motion control"],
