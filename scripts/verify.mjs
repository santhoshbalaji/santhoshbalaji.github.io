...
const productionCss = await readFile(new URL("../styles.min.css", import.meta.url), "utf8");
const productionTheme = await readFile(new URL("../theme.min.css", import.meta.url), "utf8");
const productionScript = await readFile(new URL("../script.min.js", import.meta.url), "utf8");
...
const checks = [
  ...
  [html.includes('data-theme="orbital"') && html.includes('href="theme.min.css?v=1"'), "centralized orbital theme is loaded"],
  ...
  [html.includes('id="starfield"') && script.includes("starfieldReady") && script.includes("ensureStarfield") && script.includes("scheduleStarfieldDraw") && !script.includes("animateStarfield") && !script.includes("requestAnimationFrame(animateStarfield)"), "depth-responsive starfield initializes and redraws only in response to user input"],
  ...
  [html.includes('theme.min.css?v=1') && html.includes('styles.min.css?v=1') && privacy.includes('styles.min.css?v=1'), "minified production theme assets are cache-busted"],
  [html.includes('<link rel="stylesheet" href="theme.min.css?v=1" media="print"') && html.includes('<link rel="stylesheet" href="styles.min.css?v=1" media="print"') && html.includes("this.media='all'") && html.includes('<noscript><link rel="stylesheet" href="theme.min.css?v=1" /><link rel="stylesheet" href="styles.min.css?v=1"') && html.includes('.hero h1{max-width:10.5ch') && css.includes("visibility: visible;"), "critical hero styles are inline while the complete minified theme and component stylesheets load at non-blocking priority"],
  ...
  [packageJson.dependencies?.three === "0.185.1" && threeRuntimePresent && html.includes('id="universe-render"') && html.includes('id="universe-activate"') && script.includes('import("./three-universe.js?v=37")') && script.includes("activateThreeUniverse") && script.includes("startUniverseAfterFirstPaint") && script.includes('activateThreeUniverse("automatic-primary")') && !script.includes("requestIdleCallback") && universe.includes('from "./vendor/three.module.min.js"'), "pinned, locally hosted Three.js powers the automatically started primary orrery"],
  ...
  [html.includes('data-three-state="booting"') && html.includes('data-three-activation="automatic-primary"') && html.includes('class="universe-activate"') && html.includes('script.min.js?v=5') && html.includes('class="universe-boot"') && css.includes('.gravity-stage[data-three-state="booting"] .universe-render') && css.includes('@keyframes universe-acquire') && script.includes('__portfolioUniverseBootTimer') && script.includes('gravityStage.dataset.threeState = "fallback"') && script.includes('automaticUniverseFallbackReason') && script.includes('prefersReducedMotion.matches') && script.includes('connection?.saveData') && script.includes('supportsInteractiveWebGL') && script.includes('universeActivate?.addEventListener("click"') && universe.includes('stage.dataset.threeState = "booting"') && universe.includes('stage.dataset.threeState = "fallback"') && universe.lastIndexOf('stage.dataset.threeState = "ready"') > universe.lastIndexOf('render(performance.now(), true)'), "precision WebGL starts automatically after first paint while the complete lightweight scene remains only for boot and accessible fallback recovery"],
  ...
  [earthPresent && html.includes('id="earth-render"') && html.includes('data-focus="India"') && html.includes('data-texture="assets/textures/earth-color.webp"') && html.includes("India-facing, cloud-free Blue Marble") && html.includes("orbiting an India-facing Earth") && css.includes('url("assets/textures/earth-color.webp")') && !script.includes("renderCloudlessEarth") && !script.includes("getImageData") && universe.includes("options.offsetX") && !html.includes('src="assets/earth-blue-marble.jpg"'), "Earth remains the realistic, cloud-free, India-facing hub without a main-thread pixel renderer"],
  ...
  [html.includes('script.min.js?v=5'), "minified round-robin interaction script is cache-busted"],
  [productionCss.length < css.length && productionTheme.length < theme.length && productionScript.length < script.length && productionScript.includes("three-universe.js?v=37") && productionScript.includes("automatic-primary") && productionScript.includes("data-saver") && productionScript.includes("webgl-unavailable"), "production CSS, theme, and automatic 3D interaction bundles are minified from their readable sources"],
  [universe.includes('stage.dataset.threeRendering = "adaptive-demand-driven"') && universe.includes("targetFrameInterval") && universe.includes('stage.dataset.threeFrameBudget = width < 720 ? "24fps" : "30fps"') && universe.includes("anchorScales") && !universe.includes("anchor.offsetWidth") && !universe.includes("const stageBounds = stage.getBoundingClientRect()"), "activated 3D rendering uses an adaptive frame budget and cached layout measurements"],
  [css.includes("content-visibility: auto") && css.includes("contain-intrinsic-size: auto 70rem") && css.includes("contain-intrinsic-size: auto 42rem"), "below-fold sections skip initial layout and retain stable intrinsic space"],
  [!css.includes("transition: border-color 360ms ease, box-shadow 360ms ease, opacity 360ms ease") && !css.includes("transition: background 360ms ease, box-shadow 360ms ease") && html.includes('loading="lazy" decoding="async"'), "non-composited orbit transitions are removed and below-fold marks are lazily decoded"],
