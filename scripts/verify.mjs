...
const checks = [
  ...
  [html.includes('theme.css?v=4') && html.includes('styles.css?v=48') && privacy.includes('styles.css?v=39'), "continuous background theme assets are cache-busted"],
  ...
  [packageJson.dependencies?.three === "0.185.1" && threeRuntimePresent && html.includes('id="universe-render"') && html.includes('type="module" src="three-universe.js?v=15"') && universe.includes('from "./vendor/three.module.min.js"'), "pinned, locally hosted Three.js runtime powers the 3D solar system"],
  ...
  [universe.includes("createSolarSystem") && universe.includes("new THREE.PerspectiveCamera") && universe.includes("new THREE.Raycaster") && universe.includes("ResizeObserver") && css.includes(".has-three-universe .solar-orbits { visibility: hidden; opacity: 0; }") && css.includes(".has-actual-solar-system .gravity-core { display: none; }") && script.includes('gravityStage.classList.contains("has-three-universe")') && script.includes('CustomEvent("motion:change"') && universe.includes('addEventListener("motion:change"'), "perspective 3D scene renders the solar system while preserving the established portfolio interaction shell"],
  [(universe.match(/au: \d+(?:\.\d+)?/g) || []).length === 8 && ["mercury", "venus", "earth", "mars", "jupiter", "saturn", "uranus", "neptune"].every((planet) => universe.includes(`key: "${planet}"`)) && universe.includes("Math.log1p(body.spec.au)") && universe.includes("asteroidPositions") && universe.includes("depthStars") && universe.includes('stage.dataset.threeDepth = "solar-system"') && html.includes("Solar system / 8 planets") && html.includes("Correct order · size and distance compressed"), "Sun, eight ordered planets, compressed NASA-informed orbital distances, asteroid belt, and star depth share the 3D model"],
  [universe.includes('stage.dataset.threeInteraction = "360-solar-system"') && universe.includes("activePointers") && universe.includes("pinchStartDistance") && universe.includes("pinching") && universe.includes("angularVelocityX") && universe.includes("occlusionRaycaster") && universe.includes("targetRotationY += 0.12") && !universe.includes("targetRotationY = THREE.MathUtils.clamp") && universe.includes('addEventListener("pointerdown"') && universe.includes('addEventListener("wheel"') && universe.includes('addEventListener("dblclick"') && universe.includes('addEventListener("keydown"') && universe.includes('resetButton?.addEventListener("click", resetView)') && universe.includes("IntersectionObserver") && universe.includes("rendererPixelRatio") && css.includes("touch-action: none") && css.includes(".system-object.is-occluded") && html.includes('id="universe-reset"') && html.includes("Drag 360°") && html.includes('tabindex="0"') && html.includes('role="application"'), "solar system supports unrestricted 360-degree drag and keyboard rotation, inertia, pinch and wheel zoom, depth occlusion, direct reset, adaptive rendering, hover, and mission selection"],
  ...
  [earthPresent && html.includes('id="earth-render"') && html.includes('data-focus="India"') && html.includes('data-texture="assets/earth-cloudless-surface.jpg"') && html.includes("India-facing, cloud-free Blue Marble") && script.includes("renderCloudlessEarth") && script.includes("const earthFocus = { latitude: 20.6, longitude: 78.9 }") && script.includes("const specular") && script.includes("const atmosphere") && universe.includes("texture.offset.x = 0.469") && !script.includes("centreLongitude = 48") && !html.includes('src="assets/earth-blue-marble.jpg"'), "Earth remains a realistically lit, cloud-free, India-facing Blue Marble globe inside the solar system"],
