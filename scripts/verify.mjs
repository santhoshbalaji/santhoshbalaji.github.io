...
const checks = [
  ...
  [html.includes('theme.css?v=4') && html.includes('styles.css?v=55') && privacy.includes('styles.css?v=39'), "continuous background theme assets are cache-busted"],
  ...
  [packageJson.dependencies?.three === "0.185.1" && threeRuntimePresent && html.includes('id="universe-render"') && html.includes('type="module" src="three-universe.js?v=28"') && universe.includes('from "./vendor/three.module.min.js"'), "pinned, locally hosted Three.js runtime powers the 3D product orrery"],
  ...
  [html.includes('data-three-state="booting"') && html.includes('class="universe-boot"') && html.includes('script.js?v=21') && html.includes('<noscript>') && css.includes('.gravity-stage[data-three-state="booting"] .solar-orbits') && css.includes('@keyframes universe-acquire') && script.includes('__portfolioUniverseBootTimer') && script.includes('gravityStage.dataset.threeState = "fallback"') && universe.includes('stage.dataset.threeState = "booting"') && universe.includes('stage.dataset.threeState = "fallback"') && universe.lastIndexOf('stage.dataset.threeState = "ready"') > universe.lastIndexOf('render(performance.now(), true)'), "the legacy 2D orrery is suppressed during startup until the first WebGL frame is rendered, with timed and no-script fallback recovery"],
  ...
  [html.includes('script.js?v=21'), "round-robin interaction script is cache-busted"],
