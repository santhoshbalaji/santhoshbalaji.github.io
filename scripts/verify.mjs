...
const checks = [
  ...
  [html.includes('theme.css?v=4') && html.includes('styles.css?v=52') && privacy.includes('styles.css?v=39'), "continuous background theme assets are cache-busted"],
  ...
  [packageJson.dependencies?.three === "0.185.1" && threeRuntimePresent && html.includes('id="universe-render"') && html.includes('type="module" src="three-universe.js?v=25"') && universe.includes('from "./vendor/three.module.min.js"'), "pinned, locally hosted Three.js runtime powers the 3D product orrery"],
  [universe.includes("new THREE.SphereGeometry(0.52, 128, 64)") && universe.includes("THREE.MeshPhysicalMaterial") && universe.includes("THREE.ACESFilmicToneMapping") && universe.includes("THREE.SRGBColorSpace") && universe.includes("texture.offset.x = 0.469") && universe.includes("createEarthSurfaceMaps") && universe.includes("material.roughnessMap") && universe.includes("material.bumpMap"), "India-facing Earth uses high-segment geometry, aligned surface maps, color management, and physical lighting"],
  ...
  [universe.includes('stage.dataset.threeLighting = "single-sun-physical"') && universe.includes('stage.dataset.threeSurfaces = "terrain-roughness-atmosphere"') && universe.includes("createProceduralPlanetMaps") && universe.includes("sphericalFbm") && universe.includes("createAtmosphere") && universe.includes("createRingTexture") && universe.includes("roughnessMap: maps.roughnessMap") && universe.includes("bumpMap: maps.bumpMap") && css.includes("filter: contrast(1.04) saturate(.9)"), "the 3D worlds use a restrained single-sun lighting model, terrain and roughness maps, atmospheric scattering, and physically toned presentation"],
  [universe.includes("new THREE.SpriteMaterial") && universe.includes("definition.logoScale") && universe.includes("createAtlasLogoTexture") && universe.includes("destination-out") && universe.includes("logoBackdrop") && universe.includes("createLogoBackdropTexture") && universe.includes("alphaTest: 0.035") && universe.includes("depthTest: true") && universe.includes("product.group.worldToLocal") && css.includes("visibility: hidden") && ["jurisfield", "atlas", "nammatn", "mapsmith"].every((product) => css.includes(`.has-product-orrery .object-${product} .object-logo`)), "native product marks render as camera-facing, depth-tested 3D sprites with restrained contrast support rather than projected HTML overlays"],
  [universe.includes('stage.dataset.threeInteraction = "360-product-orbits"') && universe.includes('stage.dataset.threeOcclusion = "depth-buffered-logos"') && universe.includes('stage.dataset.threeOrbitBounds = "planet-safe"') && universe.includes("edgeReserve") && universe.includes("safeOrbitExtent") && universe.includes("activePointers") && universe.includes("pinchStartDistance") && universe.includes("pinching") && universe.includes("angularVelocityX") && universe.includes("occlusionRaycaster") && universe.includes("targetRotationY += 0.12") && !universe.includes("targetRotationY = THREE.MathUtils.clamp") && universe.includes('addEventListener("pointerdown"') && universe.includes('addEventListener("wheel"') && universe.includes('addEventListener("dblclick"') && universe.includes('addEventListener("keydown"') && universe.includes('resetButton?.addEventListener("click", resetView)') && universe.includes("IntersectionObserver") && universe.includes("rendererPixelRatio") && css.includes("touch-action: none") && css.includes(".system-object.is-occluded") && html.includes('id="universe-reset"') && html.includes("Drag 360°") && html.includes('tabindex="0"') && html.includes('role="application"'), "product orrery supports unrestricted 360-degree interaction, planet-safe bounds, and precise depth-buffered occlusion"],
