...
const checks = [
  ...
  [html.includes('theme.css?v=4') && html.includes('styles.css?v=47') && privacy.includes('styles.css?v=39'), "continuous background theme assets are cache-busted"],
  ...
  [packageJson.dependencies?.three === "0.185.1" && threeRuntimePresent && html.includes('id="universe-render"') && html.includes('type="module" src="three-universe.js?v=14"') && universe.includes('from "./vendor/three.module.min.js"'), "pinned, locally hosted Three.js runtime powers the 3D universe"],
  ...
  [(universe.match(/radius: \d+\.\d+/g) || []).length === 4 && universe.includes("radius: 1.35") && universe.includes("radius: 2.62") && universe.includes("new THREE.PerspectiveCamera") && universe.includes('getPropertyValue("--orbit-x")') && universe.includes("new THREE.Raycaster") && universe.includes("ResizeObserver") && css.includes(".has-three-universe .solar-orbits { visibility: hidden; opacity: 0; }") && css.includes(".has-three-universe .gravity-core .earth-globe { visibility: hidden; opacity: 0; }") && script.includes('gravityStage.classList.contains("has-three-universe")') && script.includes('CustomEvent("motion:change"') && universe.includes('addEventListener("motion:change"'), "perspective 3D scene preserves the original orbital coordinates as its default camera composition"],
  [universe.includes("createOrbitalEnvironment") && universe.includes("new THREE.RingGeometry(0.22, 1, 192)") && universe.includes("asteroidPositions") && universe.includes("depthStars") && universe.includes('stage.dataset.threeDepth = "complete"') && universe.includes("depthFor(product.key"), "orbital planes, asteroid belt, ecliptic, and star depth share the 3D scene"],
  [universe.includes('stage.dataset.threeInteraction = "360-orbit-zoom-select"') && universe.includes("activePointers") && universe.includes("pinchStartDistance") && universe.includes("pinching") && universe.includes("angularVelocityX") && universe.includes("occlusionRaycaster") && universe.includes("targetRotationY += 0.12") && !universe.includes("targetRotationY = THREE.MathUtils.clamp") && universe.includes('addEventListener("pointerdown"') && universe.includes('addEventListener("wheel"') && universe.includes('addEventListener("dblclick"') && universe.includes('addEventListener("keydown"') && universe.includes('resetButton?.addEventListener("click", resetView)') && universe.includes("IntersectionObserver") && universe.includes("rendererPixelRatio") && css.includes("touch-action: none") && css.includes(".system-object.is-occluded") && html.includes('id="universe-reset"') && html.includes("Drag 360°") && html.includes('tabindex="0"') && html.includes('role="application"'), "3D scene supports unrestricted 360-degree drag and keyboard rotation, inertia, pinch and wheel zoom, depth occlusion, direct reset, adaptive rendering, hover, and raycast selection"],
