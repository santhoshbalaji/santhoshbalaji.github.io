...
const checks = [
  ...
  [packageJson.dependencies?.three === "0.185.1" && threeRuntimePresent && html.includes('id="universe-render"') && html.includes('id="universe-activate"') && script.includes('import("./three-universe.js?v=37")') && script.includes("activateThreeUniverse") && !script.includes("requestIdleCallback") && universe.includes('from "./vendor/three.module.min.js"'), "pinned, locally hosted Three.js powers the orrery only after explicit 3D activation"],
  ...
  [html.includes('data-three-state="preview"') && html.includes('class="universe-activate"') && html.includes('script.min.js?v=4') && html.includes('class="universe-boot"') && css.includes('.gravity-stage[data-three-state="preview"] .universe-render') && css.includes('@keyframes universe-acquire') && script.includes('__portfolioUniverseBootTimer') && script.includes('gravityStage.dataset.threeState = "fallback"') && script.includes('universeActivate?.addEventListener("click"') && universe.includes('stage.dataset.threeState = "booting"') && universe.includes('stage.dataset.threeState = "fallback"') && universe.lastIndexOf('stage.dataset.threeState = "ready"') > universe.lastIndexOf('render(performance.now(), true)'), "a complete lightweight preview loads first and precision WebGL starts only from the explicit 3D control, with timed fallback recovery"],
  ...
  [html.includes('script.min.js?v=4'), "minified round-robin interaction script is cache-busted"],
  [productionCss.length < css.length && productionTheme.length < theme.length && productionScript.length < script.length && productionScript.includes("three-universe.js?v=37"), "production CSS, theme, and interaction bundles are minified from their readable sources"],
